...
use crate::features::admin::model::{ProjectQuery,
    Vendor, VendorDto, VendorQuery, Project, ProjectDto, 
    ProjectPMDto, PMQuery, VerifyPM, VendorDropdownDto, 
    UserQuery, UserDto, UsersVendorQuery, UsersVendorDto, UsersVendor, 
...
pub async fn get_list_pm(
    ...
    match sqlx::query_as::<_, ProjectPMDto>(
                ...
                WHERE a.project_id = $1 AND a.pm_description ILIKE CONCAT('%', $2, '%')
                AND (($3 IS NULL OR $4 IS NULL) OR (a.pm_project_date >= CAST($3 AS DATE) AND a.pm_project_date <= CAST($4 AS DATE)))
